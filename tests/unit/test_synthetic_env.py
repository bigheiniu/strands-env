"""Unit tests for the synthetic environment (AWM-format)."""
...
@pytest.fixture
async def synth_env(data_dir, mock_model_factory):
    """Create a SyntheticEnv, reset it, yield it, then clean up."""
...
# ---------------------------------------------------------------------------
# SyntheticEnv.__init__ tests
# ---------------------------------------------------------------------------


class TestSyntheticEnvInit:
    ...
    def test_custom_system_prompt_overrides_default(self, data_dir, mock_model_factory):
        config = SyntheticEnvConfig(scenario=SCENARIO_NAME, task_idx=0, data_dir=data_dir)
        env = SyntheticEnv(model_factory=mock_model_factory, config=config, system_prompt="Custom prompt")
        assert env.system_prompt == "Custom prompt"

    def test_default_reward_fn_created(self, data_dir, mock_model_factory):
        config = SyntheticEnvConfig(scenario=SCENARIO_NAME, task_idx=0, data_dir=data_dir)
        env = SyntheticEnv(model_factory=mock_model_factory, config=config)
        assert isinstance(env.reward_fn, SyntheticEnvRewardFunction)
        assert env.reward_fn._env is env

    def test_custom_reward_fn_used(self, data_dir, mock_model_factory):
        config = SyntheticEnvConfig(scenario=SCENARIO_NAME, task_idx=0, data_dir=data_dir)
        custom_reward = MagicMock()
        env = SyntheticEnv(model_factory=mock_model_factory, config=config, reward_fn=custom_reward)
    ...
    def test_initial_state_before_reset(self, data_dir, mock_model_factory):
        ...
        env = SyntheticEnv(model_factory=mock_model_factory, config=config)
        assert env._tools == []
        assert env._test_client is None
        assert env.db_path is None
        assert env.initial_db_path is None
        assert env._temp_dir is None


# ---------------------------------------------------------------------------
# SyntheticEnv.reset() tests
# ---------------------------------------------------------------------------


class TestSyntheticEnvReset:
    async def test_reset_creates_db_files(self, synth_env):
        assert synth_env.db_path is not None
        assert synth_env.initial_db_path is not None
        assert synth_env.db_path.exists()
        assert synth_env.initial_db_path.exists()

    async def test_reset_db_has_correct_data(self, synth_env):
        conn = sqlite3.connect(str(synth_env.db_path))
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM users")
        assert cursor.fetchone()[0] == 2
        conn.close()

    async def test_reset_initial_db_is_snapshot(self, synth_env):
        ...
        conn = sqlite3.connect(str(synth_env.initial_db_path))
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM users")
        assert cursor.fetchone()[0] == 2
        conn.close()

    async def test_reset_creates_tools(self, synth_env):
        assert len(synth_env._tools) == 2
        tool_names = {t.tool_name for t in synth_env._tools}
        assert "list_users" in tool_names
        assert "get_user" in tool_names

    async def test_reset_creates_test_client(self, synth_env):
        assert synth_env._test_client is not None

    async def test_reset_with_explicit_db_dir(self, data_dir, mock_model_factory, tmp_path):
        db_dir = tmp_path / "explicit_db"
        config = SyntheticEnvConfig(scenario=SCENARIO_NAME, task_idx=0, data_dir=data_dir, db_dir=db_dir)
        env = SyntheticEnv(model_factory=mock_model_factory, config=config)
        await env.reset()
        try:
...
# ---------------------------------------------------------------------------
# SyntheticEnv.get_tools() tests
# ---------------------------------------------------------------------------


class TestSyntheticEnvGetTools:
    async def test_get_tools_empty_before_reset(self, data_dir, mock_model_factory):
        config = SyntheticEnvConfig(scenario=SCENARIO_NAME, task_idx=0, data_dir=data_dir)
        env = SyntheticEnv(model_factory=mock_model_factory, config=config)
        assert env.get_tools() == []

    async def test_get_tools_returns_tools_after_reset(self, synth_env):
        tools = synth_env.get_tools()
        assert len(tools) == 2

    async def test_get_tools_returns_internal_list(self, synth_env):
        assert synth_env.get_tools() is synth_env._tools


# ---------------------------------------------------------------------------
# SyntheticEnv.cleanup() tests
# ---------------------------------------------------------------------------


class TestSyntheticEnvCleanup:
    async def test_cleanup_resets_tools(self, synth_env):
        assert len(synth_env._tools) > 0
        await synth_env.cleanup()
        assert synth_env._tools == []

    async def test_cleanup_clears_test_client(self, synth_env):
        assert synth_env._test_client is not None
        await synth_env.cleanup()
        assert synth_env._test_client is None

    async def test_cleanup_clears_db_paths(self, synth_env):
        assert synth_env.db_path is not None
        await synth_env.cleanup()
        assert synth_env.db_path is None
        assert synth_env.initial_db_path is None
    ...
    async def test_cleanup_idempotent(self, synth_env):
        await synth_env.cleanup()
        await synth_env.cleanup()  # Should not raise
...
# ---------------------------------------------------------------------------
# SyntheticEnvRewardFunction.compute() tests
# ---------------------------------------------------------------------------


class TestSyntheticEnvRewardFunctionCompute:
    @staticmethod
    ...
    async def test_compute_reward_complete(self, synth_env):
        action = Action(message="do something", task_context=TaskContext())
        step_result = self._make_step_result("done")
        result = await synth_env.reward_fn.compute(action, step_result)
        assert result.reward == 1.0
        assert result.info.get("result") == "complete"

    async def test_compute_reward_others(self, synth_env):
    ...
    async def test_compute_returns_info_dict(self, synth_env):
        action = Action(message="do something", task_context=TaskContext())
        step_result = self._make_step_result("done")
        result = await synth_env.reward_fn.compute(action, step_result)
    ...
    async def test_compute_with_missing_verify_fn(self, synth_env):
        # Patch the verifier to return code without verify_task_completion
        synth_env.data_loader.get_verifier = lambda s, i: "x = 1"
        action = Action(message="do something", task_context=TaskContext())
        step_result = self._make_step_result("done")
        result = await synth_env.reward_fn.compute(action, step_result)
        assert result.reward == 0.0
    ...
    async def test_compute_with_verifier_exception(self, synth_env):
        # Patch the verifier to return code that raises
        synth_env.data_loader.get_verifier = lambda s, i: (
    ...
    async def test_compute_with_exec_syntax_error(self, synth_env):
        synth_env.data_loader.get_verifier = lambda s, i: "def broken(:\n"
        action = Action(message="do something", task_context=TaskContext())
        step_result = self._make_step_result("done")
        result = await synth_env.reward_fn.compute(action, step_result)
        assert result.reward == 0.0
    ...
    async def test_compute_passes_correct_paths(self, synth_env):
        ...
        synth_env.data_loader.get_verifier = recording_verifier
        action = Action(message="do something", task_context=TaskContext())
        ...
        assert os.environ.get("_TEST_INITIAL") == str(synth_env.initial_db_path)
        assert os.environ.get("_TEST_FINAL") == str(synth_env.db_path)
