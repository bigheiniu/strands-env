...
"""Dynamic tool generation from AWM API specifications and FastAPI implementation code.
...
def create_api_tools(
    ...
    api_spec: dict,
    db_path: Path | str,
) -> tuple[list[PythonAgentTool], TestClient]:
    ...
    for group in api_spec.get("api_groups", []):
        for endpoint in group.get("endpoints", []):
            ...
            def _make_handler(ep: dict, http_client: TestClient):
